 ...
 * @param {Object} options
 * @param {Object} options.position   - Value of position of the watermark image
...
module.exports.addWatermark = async (mainImage, watermarkImage, options) => {
    try {
        ...
        let positionX;
        let positionY;
        if (options.position.locate === 'center') {
            positionX = (main.getWidth() - newWidth) / 2;     //Centre aligned
            positionY = (main.getHeight() - newHeight) / 2;   //Centre aligned
        }
        else if (options.position.locate === 'right') {
            positionX = (main.getWidth() - newWidth);     //Centre aligned
            positionY = (main.getHeight() - newHeight) / 2;   //Centre aligned
        }
        else if (options.position.locate === 'right-bottom') {
            positionX = (main.getWidth() - newWidth);     //Centre aligned
            positionY = (main.getHeight() - newHeight);   //Centre aligned
        }
        else if (options.position.locate === 'center-bottom') {
            positionX = (main.getWidth() - newWidth) / 2;     //Centre aligned
            positionY = (main.getHeight() - newHeight);   //Centre aligned
        }
        else if (options.position.locate === 'left-top') {
            positionX = 0;     //Centre aligned
            positionY = 0;   //Centre aligned
        }
        else if (options.position.locate === 'left-bottom') {
            positionX = (newWidth) - 100;     //Centre aligned
            positionY = (main.getHeight() - newHeight);   //Centre aligned
        }
        else if (options.position.X && options.position.Y) {
        ...
        main.composite(watermark,
            positionX,
            positionY,
            Jimp.VERTICAL_ALIGN_TOP | Jimp.VERTICAL_ALIGN_MIDDLE);
...
exports.addWatermarks = async (pages, options) => {
    for (const page of pages) {
        if (options.watermarkLogo) {
            if (options.watermark?.type === 'image') {

                const watermarkOptions = {
                    // 'text': options.watermark.text,
                    // 'textSize': 8, //Should be between 1-8
                    'ratio': options.watermark.ratio ? options.watermark.ratio : 0.2,// Should be less than one
                    'opacity': options.watermark.opacity ? options.watermark.opacity : 0.2, //Should be less than one
                    'position': { locate: options.watermark.position },
                    // 'position': { X: 500, Y: 100 },
                    'dstPath': page
                };
                await this.addWatermark(page, options.watermark.path, watermarkOptions);
            }
        }
    }
}